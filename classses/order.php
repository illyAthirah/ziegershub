        ...
        public function insertOder($userId,$date){
			...
			if($getProduct){
                while($result= $getProduct->fetch_assoc()){
                    ...
                    $order_time = $date;
                    $query_order= "INSERT INTO tbl_order(productId,size,price,image,quantity,total,userId, status,order_time,recieve_time) VALUES('$productId',
                    '$size','$price','$image','$quantity','$total','$userId','0','$order_time','0')";
                    $insert_order=$this->db->insert($query_order);
                    
                }
            }
		}
        // hiển thị  Products ra trang lich su
        public function getOrderHistory($userId,$status,$date){
            $query = "SELECT od.* , pd.productName 
            FROM tbl_order AS od
            INNER JOIN product AS pd ON od.productId =pd.productId
         ...
         public function admin_getOrder($status){
            ...
            FROM tbl_order AS od
            INNER JOIN user AS us ON od.userId =us.userId
            WHERE od.status IN $status";
            $result = $this->db->select($query);
            return $result;
         }

         // hiển thị  Products ra trang admin
        public function admin_getOrder_waiting($userId,$status,$date){
            $query = "SELECT od.* , pd.productName ,username,us.userId , name,us.address,us.name,pd.brandId,pd.typeProductId
            FROM tbl_order AS od
            INNER JOIN product AS pd ON od.productId =pd.productId
            INNER JOIN user AS us ON od.userId =us.userId
         ...
         // update Quantity khi admin xác nhận Order
