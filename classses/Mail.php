...
class Mail
{
    ...
    public function sendMail($data)
    {
        ...
        if ($Email == "") {
            ...
            $sql = "SELECT * FROM user WHERE email = '$Email' ORDER BY email ASC  LIMIT 1";
            $result = $this->db->select($sql);
            if ($result === FALSE) {
                $alert = '<span class="error_Email">Address email không tồn tại</span>';
                return $alert;
            }
            ...
            if ($sendMai) {
                ...
                $sqlUpdate = "UPDATE user SET userPassword= '$hash' WHERE userId = ". $account['userId'];
