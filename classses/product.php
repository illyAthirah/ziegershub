...
class product
{
    ...
    public function insert_product($data, $files)
    {
        ...
        //kiểm tra hình Image và lấy hình Image cho vào folder uploads
        ...
        if ($productName == "" ||  $brand == "" || $typeProduct == "" ||  $category == "" ||  $product_desc == "" ||  $price == "" ||  $type == "" ||  $file_name == "" || $quantity == "") {
            ...
            return "Quantity không được âm";
        }elseif($price <0){
            return "Price không được âm";
        }else{
            $sql_check_name = "SELECT * FROM product WHERE productName = '$productName'";
            $result_check_name = $this->db->select($sql_check_name);
            if($result_check_name){
                return "Product name đã tồn tại";
            }else{
                move_uploaded_file($file_temp, $uploaded_image);
                $query = "INSERT INTO product(productName,brandId,catId,product_desc,price,type,quantity,image,typeProductId) VALUES('$productName',
    ...
    public function show_product()
    {
        $query = "SELECT pd.* , cat.catName , br.brandName , tpd.typeProductName
                ...
                INNER JOIN brand AS br ON pd.brandId=br.brandId
                INNER JOIN type_product AS tpd ON pd.typeProductId= tpd.typeProductID 
    ...
    public function getproductbyBrandId($id,$type)
    {
        ...
        FROM product AS pd INNER JOIN brand AS br ON pd.brandId=br.brandId 
    ...
    public function getproductbyTypeProductId($id,$type)
    {
        ...
        FROM product AS pd  INNER JOIN type_product AS tpd ON pd.typeProductId= tpd.typeProductID
    ...
    public function update_product($data, $files, $id)
    {
        ...
        //Kiểm tra Product name có trùng hay không
        $sql_check_name = "SELECT * FROM product WHERE productName = '$productName' AND productId NOT IN ('$id')";
        $result_check_name = $this->db->select($sql_check_name);
        ...
        if (!empty($file_name)) {
            // Nếu người dùng chọn Image
            if ($file_size > 20480) {
                $alert = "<span class='error'> Chỉ có thể tải Image dưới 20MB </span>";
                return $alert;
    ...
    // END BACKEND
    // hiện thị best seller Mens clothing
    public function getProduct_Men()
    {
        $query = "SELECT *,SUM(od.quantity) as quantitysales FROM product AS pd
    ...
    // hiện thị best seller Womens clothing
    ...
    // hiện thị best seller Kids clothing
    ...
    public function getProduct_Details($id)
    {
        $query = "SELECT pd.* , cat.catName , br.brandName
            FROM product AS pd INNER JOIN category AS cat ON pd.catId=cat.catId   
            INNER JOIN brand AS br ON pd.brandId=br.brandId
    ...
    public function searchProduct($productName)
    {
        ...
        $sql = "SELECT * FROM product WHERE productName like '%$productName%' LIMIT $item_per_page OFFSET $offsset" ;
        $result = $this->db->select($sql);
        return $result;
    }
    // Search Products theo khoảng Price
    public function searchProductPrice()
    {
        $sql = "SELECT * FROM product order by product.price desc LIMIT 20";
        $result = $this->db->select($sql);
        return $result;
    }
     // từ id kiểm tra Quantity Products còn < 0 hay không ?
     public function checkQuantityProduct($id)
     {
         $sql = "SELECT * FROM product WHERE productId = '$id' AND quantity > 0 ";
         $result = $this->db->select($sql);
         return $result;
     }
    // Search Products theo khoảng Price
    ...
    public function display_product_category($rowcategory)
    {
        ...
        $sql = "SELECT * FROM product WHERE catId IN ($rowcategory) LIMIT $tungtrang,$sp_tungtrang";
        $result = $this->db->select($sql);
        return $result;
    }
    // all product category
    public function get_all_product_category($rowcategory, $rowbrand, $rowprice){
        $sql = "SELECT * FROM product WHERE catId IN ('$rowcategory') OR brandId IN ('$rowbrand') OR price <= ('$rowprice')";
        $result = $this->db->select($sql);
        return $result;
    }
    // search brand
    ...
    {
        $sql = "SELECT * FROM product WHERE brandId IN ($rowbrand)";
        $result = $this->db->select($sql);
        return $result;
    }
    //search price
    public function display_product_price($rowprice){
        ...
        $tungtrang = ($trang-1)*$sp_tungtrang;
        $sql = "SELECT * FROM product WHERE price <= ($rowprice) LIMIT $tungtrang,$sp_tungtrang";
        $result = $this->db->select($sql);
        return $result;
    }
    // get all product price
    public function get_all_product_price($rowprice){
        $sql = "SELECT * FROM product WHERE price <= ($rowprice) ";
        $result = $this->db->select($sql);
        return $result;
    }
    ...
    public function display_product_sortprice($sort_option){
        ...
        $current_page = !empty($_GET['page'])?$_GET['page']:1;
        $offsset = ($current_page-1)*$item_per_page;
        $sql = "SELECT * FROM product  ORDER BY price $sort_option LIMIT $item_per_page OFFSET $offsset";
        $result = $this->db->select($sql);
        return $result;
    }
    // display category
...
// chọn ngẫu nhiên cac sp cung brand trừ sp hiện tại 
    ...
    {
        $query = "SELECT * FROM product where brandId='$br' and productId not in ('$id') 
    ...
    public function search_Advanced($category,$brand,$price,$search)
    {
        ...
        if($search ==""){
            ...
            if($category == '(-1)' && $brand == '(-1)' && $price !=-1){
                $sql = "SELECT * FROM product WHERE price <= '$price'  LIMIT $product_start,$sp_tungtrang";
            }elseif($category !='(-1)' && $brand =='(-1)' && $price ==-1){
                $sql = "SELECT * FROM product WHERE catId IN $category  LIMIT $product_start,$sp_tungtrang";
            }elseif($category =='(-1)' && $brand !='(-1)' && $price ==-1){
                $sql = "SELECT * FROM product WHERE brandId IN $brand  LIMIT $product_start,$sp_tungtrang";
            }elseif($category !='(-1)' && $brand !='(-1)' && $price ==-1){
                $sql = "SELECT * FROM product WHERE catId IN $category AND brandId IN $brand  LIMIT $product_start,$sp_tungtrang";
            }elseif($category !='(-1)' && $brand =='(-1)' && $price !=-1){
                $sql = "SELECT * FROM product WHERE catId IN $category AND price <= '$price'  LIMIT $product_start,$sp_tungtrang";
            }elseif($category =='(-1)' && $brand !='(-1)' && $price !=-1){
                $sql = "SELECT * FROM product WHERE brandId IN $brand AND price <= '$price'  LIMIT $product_start,$sp_tungtrang";
            }elseif($category !='(-1)' && $brand !='(-1)' && $price !=-1){
                $sql = "SELECT * FROM product WHERE catId IN $category AND brandId IN $brand AND price <= '$price'   LIMIT $product_start,$sp_tungtrang";
            }elseif($category =='(-1)' && $brand =='(-1)' && $price ==-1){
                $sql = "SELECT * FROM product  LIMIT $product_start,$sp_tungtrang";
            }

        }else{

            if($category =='(-1)' && $brand =='(-1)' && $price ==-1)
                $sql = "SELECT * FROM product WHERE productName like '%$search%'  LIMIT $product_start,$sp_tungtrang";
        }
    ...
    public function number_page($category,$brand,$price,$search)
    {
        ...
        if($search ==""){
            ...
            if($category =='(-1)' && $brand =='(-1)' && $price !=-1){
                ...
                $sql = "SELECT * FROM product WHERE catId IN $category  ";
            }elseif($category =='(-1)' && $brand !='(-1)' && $price ==-1){
                $sql = "SELECT * FROM product WHERE brandId IN $brand  ";
            }elseif($category !='(-1)' && $brand !='(-1)' && $price ==-1){
                $sql = "SELECT * FROM product WHERE catId IN $category AND brandId IN $brand ";
            }elseif($category !='(-1)' && $brand =='(-1)' && $price !=-1){
                $sql = "SELECT * FROM product WHERE catId IN $category AND price <= '$price'  ";
            }elseif($category =='(-1)' && $brand !='(-1)' && $price !=-1){
                $sql = "SELECT * FROM product WHERE brandId IN $brand AND price <= '$price'  ";
            }elseif($category !='(-1)' && $brand !='(-1)' && $price !=-1){
                $sql = "SELECT * FROM product WHERE catId IN $category AND brandId IN $brand AND price <= '$price'  ";
            }elseif($category =='(-1)' && $brand =='(-1)' && $price ==-1){
                $sql = "SELECT * FROM product ";
            }
