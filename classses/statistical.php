...
class statistical
{
    ...
    {
        $query = "SELECT COUNT(*) AS countadmin FROM admin";
        $result = $this->db->select($query);
        return $result;
    }
    public function gettongSPTheoNgay($data)
    {
        ...
        $query = "SELECT od.* , SUM(total) AS value_sumTT , SUM(od.quantity) AS value_count , pd.productName
        FROM tbl_order AS od INNER JOIN product AS pd ON od.productId =pd.productId
    ...
    {
        $query = "SELECT SUM(total) AS value_sumTT , SUM(quantity) AS value_count FROM tbl_order
